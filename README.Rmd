...
There are two ways of interfacing with `slackr` currently, that provide significantly different functionality. 

1. Using only a webook to send messages to a channel (i.e. creating a single-channel bot)
2. Creating a bot user to send messages to multiple channels, including plots, tables, files, etc. as well as deleting messages, reading the channels in a workspace, etc. (i.e. creating a fully-functional multi-channel bot)
...
And that's it! Once `slackr_setup()` has been run, you should be able to post a message with `slackr('test message')`
...
Without all of the scopes enabled, only certain functions will work. Which ones depends on which scopes you have enabled. See the function documentation for which scopes are needed.

### Known Issues

* Depending on your scopes, `slackr` could quietly fail (i.e. not throw an error, but also not post anything to your channel). If this happens, try explicitly adding the `slackr` app to your channel in your Slack workspace with `/invite @your_app_name`
